...
const Slidetwo = ({ navigation }) => {
    ...
    const getWinners = () => {
        Axios.get(`${apiurl}/mobileuser/winnerscratch`)
            .then(res => res.data)
            .then(res => {
    ...
    return (
        <Root>
            <View style={styles.top}>
                <Image style={styles.topimage} source={require('../assets/winscratch.svg')} />
            </View>
            <View style={cstyles.box}>
                {loading && <ActivityIndicator color={colors.primary} size="large" />}
                {!loading && winners.length > 0 && <FlatList
                    numColumns={4}
            ...
            <View style={[styles.row, { justifyContent: 'center' }]}>
                <Image style={styles.icon2} source={require('../assets/upto.svg')} />
            </View>
            <View style={styles.box2}>
                <View style={styles.row}>
                    <Image style={styles.icon2} source={require('../assets/next.svg')} />
                </View>
            </View>
        </Root>
    )
